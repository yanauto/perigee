  ...
  if (!tool || tool.kind !== 'tool') {
    return (
      <EmptyState
        icon="wrench"
        title="工具记录不存在"
        sub="该调用可能来自其他会话或已被清理。"
      />
    )
  }
  const statusText = tool.status === 'running' ? '执行中' : tool.status === 'done' ? '完成' : '失败'
  return (
    <div
      style={{
        flex: 1,
